...
export default function (data) {
    ...
    // 例如：-e ACTIVITIES=coupon,signin,rescue
    // 默认只创建工单系统
    ...
    activityTypes.forEach((activityType, index) => {
        ...
        try {
            ...
            if (!result || !result.success) {
                logger.error(`${activityConfig.name}创建失败: ${result?.message || '未知错误'}`);
                totalFailed++;
            } else {
            ...
            logger.error(`${activityConfig.name}创建异常: ${error.message}`);
            totalFailed++;
        }
