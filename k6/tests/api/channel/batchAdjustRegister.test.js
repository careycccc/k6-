...
import { buildChannelTeam, distributePeople, printTeamReport } from './channelInviteService.js';
...
function runWithTeam(data, tenantId, adminToken, envConfig) {
    ...
    console.log(`\n[AdjustTeam] ========== Adjust 团队模式 ==========`);
    console.log(`[AdjustTeam] 租户: ${tenantId} | 配置: ${adjustCfg.desc}`);
    ...
    if (!rootResult || rootResult.code !== 0) {
        console.error('[AdjustTeam] ❌ 总代注册失败');
        return;
    }

    console.log(`[AdjustTeam] ✅ 总代注册成功: userId=${rootResult.data.userId}`);
    regSuccessCounter.add(1, { tenant: tenantId });
