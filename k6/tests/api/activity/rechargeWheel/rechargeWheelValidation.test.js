...
function printTestSummary() {
    ...
    logger.info(`[${rechargeWheelValidationTestTag}] ========== 测试详情表格 ==========`);
    logger.info(`[${rechargeWheelValidationTestTag}] ┌────────────────┬──────────┬────────────┬──────────────────────────────────────┐`);
    logger.info(`[${rechargeWheelValidationTestTag}] │ 测试任务       │ 状态     │ 用户ID     │ 消息                                 │`);
    logger.info(`[${rechargeWheelValidationTestTag}] ├────────────────┼──────────┼────────────┼──────────────────────────────────────┤`);

    testResults.details.forEach(detail => {
        const status = detail.success ? '✅ 成功' : '❌ 失败';
        const testName = detail.testName.padEnd(14, ' ');
        const userId = (detail.userId || 'N/A').toString().padEnd(10, ' ');
        const message = detail.message.substring(0, 36).padEnd(36, ' ');

        logger.info(`[${rechargeWheelValidationTestTag}] │ ${testName} │ ${status} │ ${userId} │ ${message} │`);
    });
    ...
    if (testResults.failed > 0) {
        ...
        testResults.details.forEach(detail => {
            if (!detail.success) {
                logger.error(`[${rechargeWheelValidationTestTag}] ❌ ${detail.testName}`);
                logger.error(`[${rechargeWheelValidationTestTag}]    用户ID: ${detail.userId || 'N/A'}`);
                logger.error(`[${rechargeWheelValidationTestTag}]    错误信息: ${detail.message}`);
            }
        });
    }
