 ...
 *
 *   # 多线程多轮
 *   k6 run -e TENANT_ID=3004 -e ACCOUNT_COUNT=10 -e VUS=2 -e ITERATIONS=3 runWorkOrderFull.test.js
 *
 *   # 只触发已登录工单（调试）
 *   k6 run -e TENANT_ID=3004 -e ACCOUNT_COUNT=1 -e VUS=1 -e ITERATIONS=1 -e TRIGGER_MODE=login runWorkOrderFull.test.js
 *
...
export function setup() {
    ...
    // ---- 1. 后台查询账号并解析 userId ----
    logger.info(`[${TAG}] 查询 ${accountCount} 个账号...`);
    const userListRes = sendQueryRequest(
        { pageNo: 1, pageSize: Math.min(accountCount, 100) },
    ...
    // 取前 accountCount 个账号，获取真实账号（手机号/邮箱）
    const accountInfoList = [];
    const sliced = userListRes.list.slice(0, accountCount);

    for (const u of sliced) {
        const realAccount = getUserAccount(adminToken, u.userId);
        if (realAccount) {
            accountInfoList.push({ account: realAccount, userId: u.userId });
            logger.info(`[${TAG}] userId=${u.userId} → 账号=${realAccount}`);
        } else {
            logger.warn(`[${TAG}] ⚠️ userId=${u.userId} 无法获取真实账号，已跳过`);
        }
        sleep(0.3);
    }

    if (accountInfoList.length === 0) {
        throw new Error(`[${TAG}] 没有有效账号，终止测试`);
    }

    logger.info(`[${TAG}] 有效账号 ${accountInfoList.length} 个`);
    ...
    // ---- 3. 环境巡检（只在 setup 执行一次）----
    _runInspection(adminToken);

    return { adminToken, accountsByVu, tenantId };
}
...
export default function (data) {
    const { adminToken, accountsByVu, tenantId } = data;
    ...
    _processOrders(adminToken, tenantId, env, isMainAdmin, myUserIds);

    // ---- 阶段 C：等待所有工单清空（多轮时才需要等）----
    if (ITERATIONS > 1) {
        logger.info(`\n[${TAG}] ---- 阶段 C：等待工单清空（准备下一轮）----`);
        waitUntilAllClear(adminToken, myUserIds);
    }
...
// ============================================================
function _processOrders(adminToken, tenantId, env, isMainAdmin, userIds) {
