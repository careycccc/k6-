...
const tag = 'BatchRechargeWithdraw';
...
function withdrawSingleAccount(account, adminToken, config, index, total) {
    ...
    if (!balanceInfo || balanceInfo.balance <= 0) {
        console.warn(`[${tag}] [${index}/${total}] 余额不足，跳过提现`);
    ...
    // 5. 检查提现条件
    console.log(`[${tag}] [${index}/${total}] 步骤5: 检查提现条件...`);
    ...
    if (withdrawInfo.amountCoding !== 0) {
        ...
        return { success: false, reason: 'coding_not_complete' };
    }

    // 6. 计算提现金额（充值金额的指定比例）
    console.log(`[${tag}] [${index}/${total}] 步骤6: 计算提现金额...`);
    const withdrawAmount = Math.floor(account.rechargeAmount * config.withdrawRatio);

    if (withdrawAmount > balance) {
        console.warn(`[${tag}] [${index}/${total}] ❌ 提现金额 ${withdrawAmount} 超过余额 ${balance}，跳过`);
        return { success: false, reason: 'amount_exceeds_balance' };
    }

    console.log(`[${tag}] [${index}/${total}] ✅ 计划提现金额: ${withdrawAmount} (充值的${config.withdrawRatio * 100}%)`);
    ...
    if (!applyResult) {
        ...
        return { success: false, reason: 'apply_failed', amount: withdrawAmount };
    }
...
function printResultTable(accounts, config) {
    ...
    console.log(`[${tag}] `);
    console.log(`[${tag}] ┌────────────────────────┬──────────┬──────────┬──────────┬──────────────┐`);
    console.log(`[${tag}] │ 账号                   │ 充值金额 │ 充值方式 │ 提现金额 │ 提现状态     │`);
    console.log(`[${tag}] ├────────────────────────┼──────────┼──────────┼──────────┼──────────────┤`);
    ...
    accounts.forEach((account) => {
        ...
        console.log(`[${tag}] │ ${acc} │ ${rechargeAmt} │ ${rechargeMethod} │ ${withdrawAmt} │ ${withdrawStatus} │`);
    });
 ...
 * 打印统计信息
 */
function printStatistics(accounts, rechargeStats, withdrawStats, config) {
    console.log(`\n[${tag}] ========== 统计信息 ==========`);
...
export default function (data) {
    ...
    printStatistics(accounts, rechargeStats, withdrawStats, config);
