...
function testFieldFiltering() {
  ...
  console.log('  原始数据字段:', Object.keys(data).join(', '));
  console.log('  过滤后字段:', Object.keys(filtered).join(', '));
}
...
function testJsonSerialization() {
  ...
  console.log('  生成的 JSON:', jsonString);
  
  // 测试标准 JSON.stringify 对比
  const standardJson = JSON.stringify(testData);
  console.log('  标准 JSON.stringify:', standardJson.substring(0, 100) + '...');
}
  ...
  testCases.forEach((testCase, index) => {
    ...
    try {
      ...
      if (signedData.amount || signedData.userId) {
        ...
        console.log(`    篡改测试: ${tamperedValid ? '❌ 应该失败' : '✅ 正确失败'}`);
      }
